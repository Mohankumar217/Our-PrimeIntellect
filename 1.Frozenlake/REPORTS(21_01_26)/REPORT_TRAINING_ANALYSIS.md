...
3.  **Few-Shot Injection (Immediate):**
    *   The current `memory.json` contains mostly invalid trajectories. Manually seed `memory.json` with **one perfect, manually-written episode**.
    *   This will force the "Recall" mechanism to show the agent *correct* formatting in the context window.
